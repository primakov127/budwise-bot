...
from models import Category, FamilyExpense, Tag
...
async def on_category_selected(callback: CallbackQuery, widget: Any, manager: DialogManager, category_id: str):
    ...
    await manager.switch_to(states.AddFamilyTransaction.CONFIRM)
...
async def on_description_changed(message: Message, textInput, manager: DialogManager, description: str):
    ...
    await manager.switch_to(states.AddFamilyTransaction.CONFIRM, ShowMode.DELETE_AND_SEND)
...
add_family_transaction_dialog = Dialog(
    Window(
        ...
        Row(
            SwitchTo(Const("Category"), id="category", state=states.AddFamilyTransaction.SPECIFY_CATEGORY),
            CANCEL_MENU_BUTTON
        ),
    ...
    Window(
        ...
        state=states.AddFamilyTransaction.SPECIFY_CATEGORY,
        getter=getter,
    ),
    Window(
        ...
        SwitchTo(
            Const("Description"),
            id="description",
            state=states.AddFamilyTransaction.SPECIFY_DESCRIPTION
        ),
        SwitchTo(
            Const("Tags"),
            id="tags",
            state=states.AddFamilyTransaction.SPECIFY_TAGS
        ),
        SwitchTo(
            Const("Change date"),
            id="date",
            state=states.AddFamilyTransaction.CHANGE_DATE
        ),
        state=states.AddFamilyTransaction.CONFIRM
    ),
    Window(
